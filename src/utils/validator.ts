...
type Validator<T, U> =
  ...
  | ((
      ...
      constraints: ValidatorConstraints<U>
    ) => Validator<T, U>);
...
type ValidatorConstraintsDef<T> = Partial<
  Record<keyof T | string, Validator<Validators<T>, T>>
>;

export type ValidatorConstraints<T> =
  | ValidatorConstraintsDef<T>
  | ((fields: T) => ValidatorConstraintsDef<T>);

export function validateFields<T>(
  fields: T,
  constraints: ValidatorConstraints<T>
) {
  const resolvedConstraints =
    typeof constraints === 'function' ? constraints(fields) : constraints;

  const errors = validate(fields, resolvedConstraints) as Record<
    keyof T,
    string[] | string
  >;
...
export function validateFieldsOrThrow<T>(
  fields: T,
  constraints: ValidatorConstraints<T>
) {
  ...
  if (errors && Object.keys(errors).length) {
    const error = new Error('') as ErrorWithMeta;
